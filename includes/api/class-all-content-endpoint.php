<?php
/**
 * Sets up an endpoint to query all of the main content types.
 *
 * Adapted from:
 * https://github.com/elevati/wp-api-multiple-posttype/blob/master/lib/endpoints/class-wp-rest-multiple-posttype-controller.php
 *
...
/**
 * All Content endpoint class.
 */
class All_Content_Endpoint extends \WP_REST_Controller {
	/**
	 * Constructor
	 */
	public function __construct() {
		$this->namespace = 'mocp/v1';
		$this->rest_base = 'all_content';
	}
	 ...
	 * @return boolean
	 */
	public function has_items_permissions_check( \WP_REST_Request $request ) {
		return true;
	}
	...
	public function get_items( $request ) {
		...
		if ( empty( $args['post_type'] ) || ! is_array( $args['post_type'] ) ) {
			return new WP_Error(
				'type_need_to_be_array',
				'Type Param is a required parameter and needs to be an array: &type[]=post&type[]=page&type[]=slideshow',
				[ 'status' => 422 ]
			);
		}
	...
	protected function prepare_items_query(
		...
		// Only allow sticky posts if 'post' is one of the requested post types.
		if (
			in_array( 'post', $query_args['post_type'], true ) ||
	 ...
	 * @return array
	 */
	protected function get_allowed_query_vars( array $post_types ) {
		global $wp;
